...
- **`tester.tap()` sonrası TEK BAŞINA `pumpAndSettle()`, hiçbir animasyona/frame zamanlayıcısına bağlı OLMAYAN çıplak bir `Future.delayed` (ör. `MockPurchaseService`'in gecikmesi) İÇİN GÜVENİLİR DEĞİL** — `pumpAndSettle` "yerleşti" sanıp ERKEN dönebiliyor, `await` hiç tamamlanmadan sonraki `expect` boş çıkıyor (bkz. `AdFreeProvider`/"Zibo ADS satın alma BAŞARILI" testi — coin satın alma testindeki AYNI gecikme, ARADA Google-bağlama sheet'inin KENDİ kapanış animasyonu olduğu için tesadüfen sorunsuz çalışıyordu). Çözüm: `tap()` sonrası `await tester.pump(); await tester.pump(gecikme_süresi);` ile gecikmeyi AÇIKÇA ilerlet, ANCAK ONDAN SONRA `pumpAndSettle()` çağır.
