...
- `sendToUser` → `android.notification.channelId` — varsayılan `'push_notifications'` + `sound: 'zibo_notification'` (bkz. `dijital_kanka/android/CLAUDE.md` + `docs/decisions/010`). **Faz 5 (E2) — Zibo Pro+'a özel bildirim sesleri**: `user.proPlusSoundChoice` (`users/{uid}` KÖK alanı, `timeZone`/`lastActiveAt` ile AYNI Pattern B — istemci `PushNotificationProvider.setProPlusSoundChoice`'ta yazar) `'1'..'5'` ise `PROPLUS_SOUND_CHANNELS` haritası üzerinden `push_notifications_proplus_1..5` kanallarından birine yönlendirilir — TEK bir yerde (`sendToUser`), 5 betiğin HİÇBİRİNİN ayrıca bilmesi gerekmez. Sunucu abonelik durumunu TEKRAR doğrulamıyor (istemci-yetkili, bkz. `docs/decisions/002/005`).
